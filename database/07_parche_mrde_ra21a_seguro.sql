-- Parche seguro MRDE 611 - R.A. 2.1 A
-- Este script NO borra la actividad existente. Actualiza la actividad IOS que ya tengas
-- para conservar el id que usa el enlace actividad.html?id=1.
...
with contenido (codigo, modulo, grupo, unidad, ra, inciso, titulo, tipo, descripcion, instrucciones, evidencia_requerida, publicada, publicado, apuntes_html, diagramas_html, ejemplo_html, ejercicios_html, practica_html, reflexion_prompt) as (
  values (
  'MRDE-U2-RA21-A',
  'MRDE',
  '611',
...
set
    ...
    ra = contenido.ra,
    inciso = contenido.inciso,
    titulo = contenido.titulo,
    tipo = contenido.tipo,
    descripcion = contenido.descripcion,
    instrucciones = contenido.instrucciones,
    evidencia_requerida = contenido.evidencia_requerida,
    publicada = contenido.publicada,
    publicado = contenido.publicado,
    apuntes_html = contenido.apuntes_html,
    diagramas_html = contenido.diagramas_html,
    ejemplo_html = contenido.ejemplo_html,
    ejercicios_html = contenido.ejercicios_html,
    practica_html = contenido.practica_html,
    reflexion_prompt = contenido.reflexion_prompt,
    actualizado_en = now()
from contenido
where modulo_actividades.modulo = 'MRDE'
  and modulo_actividades.grupo = '611'
  and (
    modulo_actividades.codigo = 'MRDE-U2-RA21-A'
    or modulo_actividades.titulo ilike '%IOS%'
    or modulo_actividades.titulo ilike '%configuración básica%'
  );
...
insert into modulo_actividades (codigo, modulo, grupo, unidad, ra, inciso, titulo, tipo, descripcion, instrucciones, evidencia_requerida, publicada, publicado, apuntes_html, diagramas_html, ejemplo_html, ejercicios_html, practica_html, reflexion_prompt)
select codigo, modulo, grupo, unidad, ra, inciso, titulo, tipo, descripcion, instrucciones, evidencia_requerida, publicada, publicado, apuntes_html, diagramas_html, ejemplo_html, ejercicios_html, practica_html, reflexion_prompt
from contenido
where not exists (
  select 1
...
with repetidos as (
  ...
  from modulo_actividades
  where codigo = 'MRDE-U2-RA21-A'
)
delete from modulo_actividades
where id in (
  select id from repetidos where rn > 1
);
